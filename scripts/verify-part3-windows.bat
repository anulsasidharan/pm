...
for /f "delims=" %%i in ('powershell -NoProfile -Command "$c=(Invoke-WebRequest -Uri '%BASE_URL%/' -UseBasicParsing).Content; if($c -match 'Project Management MVP' -and $c -match 'Kanban board preview'){ 'ok' } else { exit 1 }"') do set "HTML_OK=%%i"
if %errorlevel% neq 0 (
